...
if uploaded_file:
    try:
        ...
        # 3. ЧИСТКА ДАННЫХ (Fix ошибки "can't multiply") !!!
        # Превращаем все числовые колонки в цифры, ошибки заменяем на 0
        cols_to_fix = ['Начало_Кг', 'Приход_Кг', 'Продажи_Кг', 'Конец_Кг', 'Цена_Руб']
        for col in cols_to_fix:
            df_clean[col] = pd.to_numeric(df_clean[col], errors='coerce').fillna(0)
            
        df = df_clean

        # 4. Расчеты (теперь безопасно)
